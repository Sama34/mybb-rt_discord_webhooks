...
class DiscordHelper
{
	...
	public static function logDiscordApiRequest(
	...
	public static function getDiscordMessage(int $pid, string $field = 'discord_message_id'): int
	{
		...
		$query = $db->simple_select(
			Core::get_plugin_info('prefix') . '_logs',
			$field,
	...
	public static function thirdPartyIntegration(): void
	{
		...
		$hook_data = [
			...
			'title' => '',
			// Embeds title of the content
			'url' => '',
			// Embeds url for the title
			'description' => '',
			// Embeds description
			'char_limit' => 2000,
			// Embeds max character to be parsed
			'embeds_enabled' => true,
			// Enable / Disable embeds
			'allow_mentions' => true,
			// Allow mentions in embeds (@here, @all)
			'color' => '',
			// HEX embeds color border (eg. #ff0000)
		...
		$embeds = [
			[
				...
				'description' => DiscordHelper::formatMessage(
					DiscordHelper::truncateMessage((int)$hook_data['char_limit'], $hook_data['description']),
					(bool)$hook_data['embeds_enabled']
				),
		...
		if (!empty($hook_data['webhook_url'])) {
			...
			fetch_api($hook_data['webhook_url'], $hook_data['webhook_method'], $data, $headers);
		}
	}
}